...
type NavItem = {
  ...
  iconColor?: string   // tailwind bg color class for the icon chip
}
type NavSection = { title: string; items: NavItem[]; collapsible?: boolean; comingSoon?: { label: string; icon: React.ReactNode }[] }
...
function NavLink({ href, label, icon, badge, badgeVariant, iconColor }: NavItem) {
  ...
  return (
    <Link
      href={href}
      className={`flex items-center gap-2.5 px-2.5 py-2.5 rounded-xl text-[12.5px] transition-all duration-150 ${isActive
        ? 'bg-white/[0.1] text-white font-medium'
        : 'text-slate-400 hover:text-slate-200 hover:bg-white/[0.05]'
        }`}
    >
      {/* Icon chip */}
      <span className={`w-6 h-6 rounded-lg flex items-center justify-center shrink-0 transition-all ${isActive
...
export default function Sidebar({ orgName, orgId, logoSignedUrl }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* ── Bottom block ────────────────────────────────────── */}
      <div className="shrink-0 px-2.5 py-4 space-y-1">
        <Link
          href="/perfil"
