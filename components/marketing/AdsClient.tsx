...
export default function AdsClient({
  ...
  const [cpaThreshold, setCpaThreshold] = useState(500) // MXN — configurable por el usuario
  ...
  const m = useMemo(() => {
    ...
    return {
      cost, conv, cpa, ctr, cpc, convRate,
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {connection.external_name ?? 'Cuenta conectada'} · Últimos 30 días
          </p>
        </div>
        <div className="flex items-center gap-3">
          <label className="text-xs text-slate-500">Umbral CPA objetivo:</label>
          <div className="flex items-center gap-1 border border-slate-200 rounded-lg px-3 py-1.5 bg-white">
            <span className="text-xs text-slate-400">$</span>
            <input
              type="number"
              value={cpaThreshold}
              onChange={e => setCpaThreshold(Number(e.target.value))}
              className="w-20 text-sm text-slate-700 outline-none"
            />
      ...
      {m.cpa > cpaThreshold && m.cpa > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-red-800">CPA por encima del umbral</p>
              ...
              <tbody className="divide-y divide-slate-50">
                {campaigns.length === 0 ? (
                  ...
                  <tr key={i} className="hover:bg-slate-50">
                    ...
                    <td className="py-2.5 text-right font-medium text-slate-800">{c.conv.toFixed(0)}</td>
                    <td className={`py-2.5 text-right font-medium ${c.status === 'bad' ? 'text-red-600' : c.status === 'warn' ? 'text-amber-600' : 'text-emerald-600'}`}>
...
function KpiCard({ label, value, delta, positiveIsGood, sub, highlight }: {
  ...
  return (
    <div className={`rounded-2xl border p-5 ${highlight ? 'border-red-200 bg-red-50' : 'border-slate-200 bg-white'}`}>
      <p className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-3">{label}</p>
      <p className={`text-3xl font-bold mb-1 ${highlight ? 'text-red-700' : 'text-slate-900'}`}>{value}</p>
