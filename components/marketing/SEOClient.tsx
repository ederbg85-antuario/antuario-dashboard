...
function avgM(rows: MetricRow[], key: string) {
  const filtered = rows.filter(r => r.metric_key === key)
...
export default function SEOClient({
  connection, globalMetrics, prevMetrics, trendData,
  topKeywords, topPages, opportunityKeywords,
}: Props) {
  ...
  const m = useMemo(() => {
    ...
    const prevClicks      = sumM(prevMetrics, 'clicks')
    const prevImpressions = sumM(prevMetrics, 'impressions')
    const prevCtr         = avgM(prevMetrics, 'ctr') * 100
    const prevPos         = avgM(prevMetrics, 'position')

    return {
      clicks, impressions, ctr, position,
      deltaClicks:  calcDelta(clicks, prevClicks),
      deltaImpr:    calcDelta(impressions, prevImpressions),
      deltaCtr:     calcDelta(ctr, prevCtr),
      deltaPos:     calcDelta(position, prevPos),
    }
  }, [globalMetrics, prevMetrics])

  // Agrupar tendencia por fecha
  const trend = useMemo(() => {
    const map: Record<string, { date: string; clicks: number; impressions: number; position: number }> = {}
    trendData.forEach(r => {
      if (!map[r.date]) map[r.date] = { date: r.date, clicks: 0, impressions: 0, position: 0 }
  ...
  // Agregar keywords por dimensión
  const kwMap = useMemo(() => {
  ...
  // Oportunidades: alta impresión, CTR bajo
  const opportunities = useMemo(() => {
    return opportunityKeywords
      .filter(r => r.dimension_value)
      .slice(0, 10)
      .map(r => ({
        kw:          r.dimension_value ?? '—',
        impressions: r.value,
        estimatedClicks: Math.round(r.value * 0.03), // si mejoras a 3% CTR
      }))
  }, [opportunityKeywords])

  if (!connection) {
    return <ConnectCTA source="search_console" label="Google Search Console" />
  }

  if (!hasData) {
    return <NoData label="Search Console" lastSync={connection.last_sync_at} />
  }

  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-xl font-bold text-slate-900">SEO — Search Console</h1>
        <p className="text-sm text-slate-500 mt-0.5">
          {connection.external_name ?? 'Propiedad conectada'} · Últimos 30 días
        </p>
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-4 gap-4">
        <KpiCard label="Clics orgánicos"   value={fmtN(m.clicks)}      delta={m.deltaClicks}  positiveIsGood sub="búsquedas que llegan a tu sitio" />
        <KpiCard label="Impresiones"        value={fmtN(m.impressions)} delta={m.deltaImpr}    positiveIsGood sub="veces que apareciste en Google" />
        <KpiCard label="CTR promedio"       value={`${m.ctr.toFixed(1)}%`}  delta={m.deltaCtr} positiveIsGood sub="de impresiones → clics" />
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6">
        <p className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-6">
          Clics vs Impresiones — últimos 6 meses
        </p>
      ...
      <div className="grid grid-cols-2 gap-4">
          ...
          <div className="overflow-auto max-h-80">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {kwMap.length === 0 ? (
                  <tr><td colSpan={5} className="py-8 text-center text-slate-400 text-xs">Sin datos de keywords</td></tr>
                ) : kwMap.map((kw, i) => (
                  <tr key={i} className="hover:bg-slate-50">
                    <td className="py-2 pr-4 text-slate-700 truncate max-w-[180px]">{kw.kw}</td>
                    <td className="py-2 text-right font-medium text-emerald-700">{fmtN(kw.clicks)}</td>
        ...
        <div className="bg-white rounded-2xl border border-amber-100 p-6">
          ...
          <p className="text-xs text-slate-500 mb-4">
            Keywords con muchas impresiones pero CTR bajo — mejorar su título puede generar clics sin costo extra.
          </p>
          {opportunities.length === 0 ? (
            <p className="text-sm text-slate-400 py-8 text-center">Sin oportunidades identificadas</p>
          ) : (
            <div className="space-y-2">
              {opportunities.map((op, i) => (
                <div key={i} className="flex items-center justify-between p-3 bg-amber-50 rounded-xl">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-slate-700 truncate">{op.kw}</p>
                    <p className="text-xs text-slate-400 mt-0.5">{fmtN(op.impressions)} impresiones</p>
                  </div>
                  <div className="text-right shrink-0 pl-3">
                    <p className="text-xs text-amber-700 font-semibold">+{fmtN(op.estimatedClicks)} clics pot.</p>
...
function ConnectCTA({ source, label }: { source: string; label: string }) {
  return (
...
function NoData({ label, lastSync }: { label: string; lastSync: string | null }) {
  return (
    <div className="flex flex-col items-center justify-center py-32 text-center px-8">
      ...
      <h3 className="text-lg font-bold text-slate-700 mb-2">{label} conectado — sync pendiente</h3>
