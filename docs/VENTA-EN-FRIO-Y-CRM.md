...
- **Sección Ventas → Prospección** (`/ventas/prospeccion`): workspace de venta en frío. Componentes: `components/ventas/ProspeccionClient.tsx` (workspace) + `components/ventas/EmbudoMetricas.tsx` (pestaña Métricas). Carga en `app/(dashboard)/ventas/prospeccion/page.tsx` (service_role). Multi-vendedor: `prospects.assigned_to` (uuid → profiles). Vista Mis/Todo el equipo.
