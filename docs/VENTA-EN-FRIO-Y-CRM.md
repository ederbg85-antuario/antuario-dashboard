...
- **Sección Ventas → CRM** (`/ventas/crm`) = EL workspace unificado (jul-2026): `components/ventas/CrmClient.tsx` + `components/ventas/EmbudoMetricas.tsx`. Unifica `prospects` (frío) + `contacts` (warm) + `proposals` en un pipeline de 6 columnas (Por contactar → Contactado → Interesado → Reunión → Propuesta → Cliente) con: tablero **drag & drop** (arrastrar tarjeta = cambia stage/contact_type; prospecto no puede ir a Propuesta/Cliente sin convertirse a contacto), lista, pestaña **Métricas** (pipeline general + detalle frío), ficha lateral **editable** (etapa, camino, vendedor, próxima acción, necesidad/notas, bitácora, compositor de plantillas, convertir, descartar, captura de decisor), alta manual y agenda de pendientes. `/ventas/prospeccion` ahora solo **redirige** a `/ventas/crm` (ProspeccionClient fue eliminado).
